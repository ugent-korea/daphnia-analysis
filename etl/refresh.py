...
DB_URL    = os.environ["DATABASE_URL"]  # e.g. postgresql://user:pass@host/db?sslmode=require
...
def _write_mothers(conn, df: pd.DataFrame):
    # Always have the right columns/order
...
def main():
    ...
    # --- Read from Google Sheets exactly like before ---
    gc = _authorize()
    ...
    # --- Build mothers BEFORE hashing/writing (prevents NameError) ---
